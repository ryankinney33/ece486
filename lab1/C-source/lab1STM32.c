/*
 * Adapted from the Example program to illustrate the use of the ECE 486 interface.
 *
 * An input waveform is copied to the primary output DAC. The waveform is then
 * passed through the running_mean filter and streamed to the second DAC.
 * USER button presses invert the signal on the primary DAC.
 *
...
int main(void)
{
  ...
  struct running_mean runMeanFilter;
  char lcd_str[8];
  ...
  // the MSI was used because the board was not modified to use the HSE
  ...
  runMeanFilter = init_running_mean(M,nsamp);
  ...
  while(1){
    ...
    // filter output 2 through the running_mean filter
    calc_running_mean(input,&runMeanFilter,output2);
